...
3. Update and raise containers using `TAG=[new version] docker compose -f docker-compose-prod.yml up -d`
...
5. If it didn't work, don't let it run in a broken state. Lower the containers again `TAG=[new version] docker compose -f docker-compose-prod.yml down`.
